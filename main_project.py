                ...
                try:
                    ...
                    if len(img_list) >= 2:
                        if img_idx == len(img_list):
                            print("证明图片已经用完了,没有找到,直接退出就行了")
                            break;
                        while current_idx < 5:
                            img_path = get_resource_path(img_list[img_idx])
                        ...
                        img_path = get_resource_path(img)
                        location = pyautogui.locateCenterOnScreen(img_path, confidence=0.9)
                        if location is not None:
                            # 证明我找到了,那么就可以操作
                            pyautogui.click(location.x, location.y, clicks=clickTime,interval=0.2,duration=0.2,button="left")
...
def mainWork(sheetdata):
    ...
    while i < sheetdata.nrows:
        ...
        if opacity_value == 1:
            ...
            print(copy_value.value)
            print(copy_value.ctype)
            # 这个地方文本数字都要复制
            if copy_value.ctype == 1:
                pyperclip.copy(copy_value.value)
                i += 1
                continue
            ...
            # 等待
            # 强制转数字
            time.sleep(int(sheetdata.row(i)[1].value))
            ...
            while isGoing:
                ...
                while operator_index < len(operator_item):
                    ...
                    if len(operator_item[operator_index].split(' ')) >= 2:
                        ...
                        try:
                            img_path = get_resource_path(operator_item[operator_index])
...
if __name__ == '__main__':
    ...
    # 自动选择速卖通海关信息
    # 速卖通自动给客户发消息
    # file = 'aliexpress_msg.xls'
    file = get_resource_path('temu_auto.xls')
    #打开文件
